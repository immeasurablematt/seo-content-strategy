...
**WRITING PROCESS:** Complete sections 2-5 first, THEN write Section 1 (Executive Summary) to summarize the article you just briefed.
...
11. Research & Sources to Include"""
    ...
    try:
        ...
        try:
            ...
            return f"Error generating brief: {str(e2)}"
...
with col1:
    ...
    use_auto_recommend = st.checkbox(
        "🤖 Auto-recommend audience & goal (analyzes top 4 competitors)",
        value=True,
        help="Let AI analyze competitors and suggest optimal targeting"
    )
    ...
    if not use_auto_recommend:
        ...
        st.info("💡 Audience and goal will be auto-recommended after analyzing competitors")
...
if st.button("🚀 Generate SEO Brief", type="primary", use_container_width=True):
    if not primary_keyword:
    ...
    elif not use_auto_recommend and (not target_audience or not content_goal):
        st.error("⚠️ Please fill in audience and goal, or enable auto-recommend")
    else:
        ...
        try:
            ...
            status_text.text("🔍 Step 2/7: Analyzing SERP results & domain authority...")
            ...
            # Step 3: Auto-recommend audience/goal
            if use_auto_recommend:
                status_text.text("🤖 Step 3/7: Analyzing competitors to recommend audience/goal...")
                progress_bar.progress(42)
                audience_goal_rec = analyze_competitor_audience_and_goals(serp_results, primary_keyword)
                target_audience = audience_goal_rec['target_audience']
                content_goal = audience_goal_rec['content_goal']
                
                st.info(f"**Recommended Audience:** {target_audience}\n\n**Recommended Goal:** {content_goal}\n\n*Rationale: {audience_goal_rec['rationale']}*")
            else:
                audience_goal_rec = None
                progress_bar.progress(42)
            
            # Step 4: Competition Assessment + Content Length
            status_text.text("🎯 Step 4/7: Analyzing competition and measuring content lengths...")
            ...
            status_text.text("🔗 Step 5/7: Finding relevant internal links...")
            ...
            if internal_pages:
                internal_links = find_relevant_internal_links(
                    primary_keyword, target_audience, content_goal, internal_pages, 5
                )
            else:
                internal_links = []
                st.warning("⚠️ No internal pages CSV found - skipping internal links")
            
            # Step 6: Generate Brief
            status_text.text("🤖 Step 6/7: Generating comprehensive content brief...")
            ...
            st.session_state.content_goal = content_goal
            st.session_state.audience_goal_rec = audience_goal_rec if use_auto_recommend else None
...
if st.session_state.brief_generated:
    ...
    if st.session_state.competition_assessment:
        ...
        with col2:
            verdict = st.session_state.competition_assessment['verdict']
            st.metric(
                "Verdict",
                verdict.split(' - ')[0] if ' - ' in verdict else verdict
            )
        with col3:
            st.metric(
                "Target Position",
        ...
        with col4:
            content_length = st.session_state.competition_assessment.get('content_length_analysis', {})
            target_words = f"{content_length.get('recommended_min', 0):.0f}-{content_length.get('recommended_max', 0):.0f}"
            st.metric(
                "Target Length",
    ...
    content_length_data = st.session_state.competition_assessment.get('content_length_analysis', {})
    recommended_min = content_length_data.get('recommended_min', 0)
    recommended_max = content_length_data.get('recommended_max', 0)
    
    rationale = ""
    if hasattr(st.session_state, 'audience_goal_rec') and st.session_state.audience_goal_rec:
        rationale = st.session_state.audience_goal_rec.get('rationale', '')
...
**Content Goal:** {st.session_state.content_goal}
**Primary Keyword:** {primary_keyword}
**Target Length:** {recommended_min:.0f}-{recommended_max:.0f} words{rationale_section}
...
*Generated by SEO Brief Generator powered by Claude AI + DataForSEO*
"""
